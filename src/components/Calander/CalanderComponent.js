...
const CalanderComponent = props => {
  ...
  const makeInputToPriceFormat = props.makeInputToPriceFormat;
  ...
  const savingResData = () => {
    props.getDateRange(resStartDate, resEndDate, betweenResDate);
    if (
      betweenResDate.length >
      findUniqElem(betweenResDate, props.excludeDateArray).length
    ) {
      alert(
      ...
      // console.log(resStartDate, resEndDate, numberOfGuests);
    }
  };

  const excludeDateArray = props.excludeDateArray.map(date => {
    return addDays(new Date(date), 0);
  });

  const findUniqElem = (arr1, arr2) =>
    [...arr1, ...arr2].filter(item => !arr2.includes(item));
  let totalPrice = props.price * reservedNight;
  ...
  const sendData = () => {
    fetch(`http://10.58.52.210:3000/reservation/${props.productId}`, {
      method: 'POST',
      headers: {
  ...
  return (
    <S.CalanderAndPeopleNumberCheckContainer>
      ...
      <S.ComputingArea>
        <S.PriceForNight>
          <S.InfoForprice>
            ₩ {makeInputToPriceFormat(props.price)}원 x {reservedNight}박
          </S.InfoForprice>
          <S.ComputedPrice>
